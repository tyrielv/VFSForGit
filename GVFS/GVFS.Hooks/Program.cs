...
namespace GVFS.Hooks
{
    public class Program
    {
        ...
        private static void HandleWorktreeRemove(string[] args)
        {
            ...
            if (!hasForce)
            {
                // VFS worktrees require --force because we must unmount ProjFS
                // before git can delete the directory. Check for uncommitted
                // changes to give a helpful message.
                ...
                if (!string.IsNullOrWhiteSpace(statusResult.Output))
                {
                    Console.Error.WriteLine(
                        $"error: worktree '{fullPath}' has uncommitted changes and is managed by VFS for Git.\n" +
                        $"Use 'git worktree remove --force' to remove it anyway.");
                }
                else
                {
                    Console.Error.WriteLine(
                        $"error: VFS for Git worktrees require --force to remove.\n" +
                        $"Use 'git worktree remove --force {worktreePath}'");
                }
